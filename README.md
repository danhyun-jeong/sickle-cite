...
1. **신기능**
	1. 이제 추출한 논문 서지정보에 더해 사용자가 직접 ‘🏷️ 프로젝트 태그’를 달 수 있습니다.
		- [📑 현재 페이지 서지정보] 탭에서 ‘태그’를 입력할 수 있습니다. 입력창에 텍스트를 입력하고 Enter 키를 누르면 칩 형태의 UI가 적용되면서 태그가 추가됩니다. Delete 키를 누르거나 각 태그 칩에 있는 삭제 버튼(×)을 클릭해 삭제할 수 있습니다.
		- 태그를 추가하거나 삭제할 때 자동으로 내역(히스토리)에도 저장됩니다.
		- [🕰️ 서지정보 추출 내역] 탭의 표에 ‘태그’ 열이 추가되었습니다. 각 논문에 달아놓은 태그를 한눈에 확인할 수 있습니다.
		- 태그 역시 검색 가능합니다. 내역에 서로 다른 목적으로 찾아본 논문들이 뒤섞여 있을 때, 필터링을 적용할 수 있는 셈입니다.
	2. 본 확장 프로그램을 실행해 팝업이 열림에 따라 현재 페이지에서 논문 서지정보를 추출할 때, 동일한 논문의 서지정보를 이전에 추출한 적이 있으면 이점을 토스트 알림 메시지로 알려줍니다.
		- 이때, 이미 이전에 추출한 논문 서지정보에 사용자가 태그를 달아두었을 경우, 그 태그 또한 가져옵니다.
	3. **(베타 테스터의 피드백을 반영)** 이제 인용 표기에 논문의 전체 쪽수 범위 표기를 포함시킬 것인지 여부를 설정할 수 있습니다. ([⚙️ 설정] 탭)
		- 쪽수 범위 구분자를 커스텀할 수 있습니다. (‘–’를 쓸 것인지 ‘~’를 쓸 것인지 등)
		- 단위를 커스텀할 수 있습니다. (‘쪽’을 쓸 것인지 ‘면’을 쓸 것인지 등)
2. **UI 개선**
...
3. 기타 오류 수정
	- 논문 제목 내에서 따옴표로 감싸진 하이픈은 본제목과 부제목을 나누는 구분자로 오인되지 않게 처리
...
- (신버전 기능) 추출한 논문 서지정보에 더해 사용자가 직접 🏷️ 태그를 달아 논문을 분류할 수 있습니다.
- 추출된 서지정보는 자동으로 ‘🕰️ 내역(히스토리)’에 저장됩니다. 이렇게 저장된 서지정보는(태그 포함) 🔍 검색 가능하며, 엑셀 파일로 ⬇️ 다운로드할 수도 있습니다.
...
<p align="center"><img width="651" alt="Image" src="https://github.com/user-attachments/assets/ada00b94-374e-4591-bbdf-058a6c6fc999" /></p>
...
8. 하단 버튼
	- 맨 아래에는 왼쪽부터 차례대로 [내역 전체 삭제] 버튼, [서지정보 내역을 엑셀로 다운로드] 버튼, [같은 논문이 중복될 경우 첫 번째만 표시] 토글 스위치가 있습니다.
		1. [내역 전체 삭제] 버튼
			- 이 버튼을 누를 경우, 말 그대로 저장되어 있는 내역 전체를 지웁니다. 다만 잘못 누른다고 곧바로 참사가 일어나진 않습니다. 버튼을 누르면 진짜 삭제할 것인지를 확인 창을 띄워 다시 한번 물어봅니다. 여기서 ‘예’를 누르신 후라면, 복원은 불가능합니다.
			- 사용자가 삭제를 누르지 않는 한, 기본적으로 내역은 계속해서 쌓입니다. (물론 완전히 무제한인 것은 아니고, 이론적으로 계산했을 때 최신 버전의 크롬이라면 약 5000개 가량 저장 가능할 것으로 예상됩니다.) 다만, 확장 프로그램을 삭제할 경우 이 내역도 같이 삭제됩니다. 오류 또는 실수 등으로 서지정보 내역이 손실될까봐 걱정되실 경우에는, [서지정보 내역을 엑셀로 다운로드] 버튼을 눌러 내역을 파일로 저장해두시면 됩니다.
		2. [서지정보 내역을 엑셀로 다운로드] 버튼
			- 이 버튼을 누르시면, 저장되어 있는 내역 전체를 엑셀로 저장합니다. 물론 [탭 2]에서 보여지는 정보만 저장되는 것이 아니고, 모든 서지정보(metadata), 사용자가 추가한 프로젝트 태그, 최종 조합된 인용 표기(citation)에 더해 추출한 학술 DB 종류, 추출한 페이지 URL(링크), 추출한 일시 정보까지 저장됩니다.
		3. [같은 논문이 중복될 경우 첫 번째만 표시] 토글 스위치
			...
			- 이 토글 스위치는 [서지정보 내역을 엑셀로 다운로드] 기능에도 영향을 미칩니다. on 되어 있으면 중복된 같은 논문은 맨 이른 시간 추출된 항목만 포함됩니다. off 되어 있으면 중복된 항목까지 전부 포함됩니다.
